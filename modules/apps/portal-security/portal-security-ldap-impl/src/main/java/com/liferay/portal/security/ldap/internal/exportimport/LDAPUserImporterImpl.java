...
public class LDAPUserImporterImpl implements LDAPUserImporter {
	...
	private String _updateUserPassword(
		...
		boolean importUserPasswordEnabled =
			ldapImportConfiguration.importUserPasswordEnabled();
		boolean forceAutogenerate = false;

		if (importUserPasswordEnabled && FIPSModeUtil.isEnabled() &&
			!FIPSModeUtil.isApprovedPasswordAlgorithm(
				PropsUtil.get(PropsKeys.PASSWORDS_ENCRYPTION_ALGORITHM))) {

			if (_log.isWarnEnabled()) {
				_log.warn(
					"Ignoring importUserPasswordEnabled because FIPS mode " +
						"is enabled and passwords.encryption.algorithm is " +
							"not FIPS-approved; using an autogenerated " +
								"password for user " + screenName);
			}

			importUserPasswordEnabled = false;
			forceAutogenerate = true;
		}

		if (!importUserPasswordEnabled) {
