...
public class FIPSLDAPSSLSocketFactoryTest {
	...
	public void testAllowlistIsOnlyAEADSuites() {
		for (String suite : FIPSLDAPSSLSocketFactory.FIPS_CIPHER_SUITES_ALLOWLIST) {
			Assert.assertTrue(
	...
	public void testEnabledProtocolsAreTLS12AndTLS13() {
		Set<String> protocols = new HashSet<>(
			Arrays.asList(FIPSLDAPSSLSocketFactory.ENABLED_PROTOCOLS));

		Assert.assertEquals(2, protocols.size());
	...
	public void testIntersectKeepsOnlySupportedDesiredValues() {
		...
		String[] intersection = FIPSLDAPSSLSocketFactory.intersect(
			desired, supported);
	...
	public void testIntersectReturnsEmptyWhenNoneMatch() {
		...
		Assert.assertEquals(0, intersection.length);
	}
