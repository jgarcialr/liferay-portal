...
boolean disableImportUserPassword = FIPSModeUtil.isEnabled() && !FIPSModeUtil.isApprovedPasswordAlgorithm(PropsUtil.get(PropsKeys.PASSWORDS_ENCRYPTION_ALGORITHM));
%>
...
<aui:fieldset>
	...
	<aui:input disabled="<%= disableImportUserPassword %>" helpMessage='<%= disableImportUserPassword ? "enable-user-password-on-import-fips-disabled-help" : null %>' label="enable-user-password-on-import" name='<%= "ldap--" + LDAPConstants.IMPORT_USER_PASSWORD_ENABLED + "--" %>' type="checkbox" value="<%= !disableImportUserPassword && ldapImportConfiguration.importUserPasswordEnabled() %>" />
